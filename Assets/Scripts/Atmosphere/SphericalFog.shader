...
Shader "FX/Spherical Fog" {
	...
	Category {
		...
		SubShader {
			...
			inline float CalcVolumeFogIntensityMINE(
							float3 sphereCenter,
							float sphereRadius,
							float innerRatio,
							float density,
				...
				// Local is the cam position in local space of the sphere.
				float3 local = cameraPosition - sphereCenter;
				float3 sunDir = normalize(sunPosition - sphereCenter);
				...
				float camPercent = (length(local)/sphereRadius);

				float camBlend = blend(0.8, 1, camPercent, 2);

				// Initially there's no fog, which is full clarity.
				float clarity = 1;
				for (int seg = 0; seg < 10; seg++)
				{
					float3 position = local + viewDirection * sample; // position of sample

					float3 dir = normalize(position);

					float strength = (cos( acos(dot(dir, sunDir)) )+1)/2;

					float val = saturate(centerValue * (1 - length(position) / sphereRadius))*strength;
				...
				float3 dir = normalize(local + viewDirection * backDepth);
				float strength = (cos(acos(dot(dir, sunDir))) + 1) / 2;

				float inner = 0.5 * (backDepth / dist2);

				if (backDepth == dist2) inner = 0.8;

				inner *= strength;

				return ((1 - clarity) * (1 - camBlend) + camBlend * inner);//*strength;
			}

			float planetRadius;

			float densityAtPoint(float3 densitySamplePoint, float3 planetCentre, float atmosphereRadius) {
				...
				half4 frag (v2f i) : COLOR {
					...
					float fog = CalcVolumeFogIntensityMINE(
						FogParam.xyz * 0.001,
						FogParam.w * 0.001,
						_InnerRatio,
						_Density * 1000,
